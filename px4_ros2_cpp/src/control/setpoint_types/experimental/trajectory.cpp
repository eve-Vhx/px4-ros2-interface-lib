...
void TrajectorySetpointType::update(
  ...
  const std::optional<Eigen::Vector3f> & velocity_ned_m_s,
  ...
  std::optional<float> yaw_rate_ned_rad_s)
{
  ...
  Eigen::Vector3f velocity = velocity_ned_m_s.value_or(Eigen::Vector3f{NAN, NAN, NAN});
  sp.velocity[0] = velocity.x();
  sp.velocity[1] = velocity.y();
  sp.velocity[2] = velocity.z();
